...
def main(to_put, to_delete, to_get, size):  # returns tuple(list, hash table)
    """
    Simulates a hash table by performing put, delete, and get operations in sequence.
    ...
    Returns:
        tuple: A tuple containing:
               - list: Values retrieved for each key in to_get (None if key not found).
               - hashtable: The final hash table after all operations.
    """
    ...
    for x in to_put:
        put(hashtable,x[0],x[1],size)
    print("hashtable",hashtable)
    for x in to_delete:
        delete(hashtable,x,size)
    lis=[]
    for x in to_get:
        value=get(hashtable,x,size)
        lis.append(value)
    return (lis,hashtable)
