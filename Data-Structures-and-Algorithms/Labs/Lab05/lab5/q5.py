def sort_rectangles(rectangle_records, record_title):
    ...
    Parameters:
    rectangle_records (array of dicts): Fixed-size array of rectangle dictionaries to be sorted.
    record_title (str): Key to sort by (e.g., "ID", "Length", "Breadth", "Color").

    Returns:
    array of dicts: Sorted rectangle records.
    """
    ...
    for i in range(1,n):
        current_elem=rectangle_records[i]
        j=i-1
        while j>=0 and rectangle_records[j][record_title]>current_elem[record_title]:
            rectangle_records[j+1]=rectangle_records[j]
            j-=1
        rectangle_records[j+1]=current_elem
